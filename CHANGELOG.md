...
- Update default PR reviewers from separate accounts to the `carsdotcom/fe-dev-sbm` team group.
...
- **Code Review**: Exported missing `LandRoverHandler` in `sbm/oem/__init__.py`.
- **Code Review**: Added missing `r"land rover"` matching support to `LandRoverHandler.get_brand_match_patterns()`.
- **Code Review**: Appended tests asserting `OEMFactory` routes to mapping classes by `dealer_info={"brand": "Land Rover"}` properly.
...
- **Reviewer Group Fix**: Replaced bugged `fe-dev-sbm` group with individual reviewers (`etritt-cc`, `messponential`, `abond-cc`, `tcollier-di`, `ssargent-cc`) in the SBM tool configuration and help text.

## [2.15.4] - 2026-02-14

### Changed

- Fix lines_migrated to always match GitHub PR additions count
...
- require FIREBASE\_\_API_KEY during setup/doctor for authenticated stats access
