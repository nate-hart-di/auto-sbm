...
class TestSetupScriptAliases:
    ...
    def test_aliases_use_project_root_variable(self):
        ...
        assert 'alias sbm-dev="cd $PROJECT_ROOT' in content, "Aliases don't use $PROJECT_ROOT"
    ...
    def test_aliases_use_unquoted_heredoc(self):
        ...
        alias_section = re.search(r"cat >> \"\$ZSHRC_FILE\" << ZSHRC_EOF", content)
...
class TestSetupScriptWrapperGeneration:
    ...
    def test_wrapper_includes_environment_isolation(self):
        ...
        wrapper_section = re.search(
            r'cat > "\$WRAPPER_PATH" << EOF.*?^\s*EOF', content, re.DOTALL | re.MULTILINE
        )

        assert wrapper_section is not None, "Wrapper generation section not found"
        wrapper_text = wrapper_section.group(0)
        print(f"DEBUG: Wrapper text extracted:\n{wrapper_text}")

        # Should unset VIRTUAL_ENV
        assert "unset VIRTUAL_ENV" in wrapper_text, "Wrapper doesn't unset VIRTUAL_ENV"

        # Should unset PYTHONPATH
        assert "unset PYTHONPATH" in wrapper_text, "Wrapper doesn't unset PYTHONPATH"

        # Should unset PYTHONHOME
        assert "unset PYTHONHOME" in wrapper_text, "Wrapper doesn't unset PYTHONHOME"
    ...
    def test_wrapper_includes_path_cleanup(self):
        ...
        wrapper_section = re.search(
            r'cat > "\$WRAPPER_PATH" << EOF.*?^EOF', content, re.DOTALL | re.MULTILINE
        )
        ...
        assert "export PATH=" in wrapper_text, "Wrapper doesn't export PATH"
    ...
    def test_wrapper_uses_absolute_paths(self):
        ...
        assert 'VENV_PYTHON="$VENV_PYTHON"' in wrapper_text or 'VENV_PYTHON="' in wrapper_text, (
            "Wrapper doesn't define VENV_PYTHON"
        )

        # Should define PROJECT_ROOT with absolute path
        assert 'PROJECT_ROOT="$PROJECT_ROOT"' in wrapper_text or 'PROJECT_ROOT="' in wrapper_text, (
...
class TestSetupScriptEnvironmentConfig:
    ...
    def test_setup_creates_local_bin_directory(self):
        ...
        assert 'mkdir -p "$LOCAL_BIN_DIR"' in content or 'mkdir -p "$HOME/.local/bin"' in content, (
    ...
    def test_setup_adds_local_bin_to_path(self):
        ...
        # Should add to PATH
        assert (
            'export PATH="$HOME/.local/bin:$PATH"' in content
            or 'export PATH="$LOCAL_BIN_DIR:$PATH"' in content
    ...
    def test_setup_checks_path_not_already_added(self):
        ...
        assert "if ! grep -q" in content, "setup.sh doesn't check before adding to .zshrc"
...
class TestSetupScriptGitShortcuts:
    ...
    def test_setup_includes_git_shortcuts(self):
        ...
        for alias in git_aliases:
            assert f"alias {alias}" in content, f"setup.sh missing git alias: {alias}"
...
class TestSetupScriptSbmAliases:
    ...
    def test_sbm_dev_alias_exists(self):
        ...
        assert "alias sbm-dev=" in content, "sbm-dev alias not found in setup.sh"
    ...
    def test_sbm_dev_alias_uses_project_root(self):
        ...
        assert "$PROJECT_ROOT" in sbm_dev.group(1), "sbm-dev doesn't use $PROJECT_ROOT variable"
    ...
    def test_sbm_test_alias_exists(self):
        ...
        assert "alias sbm-test=" in content, "sbm-test alias not found in setup.sh"
    ...
    def test_sbm_test_alias_uses_project_root(self):
        ...
        # Should use $PROJECT_ROOT
        assert "$PROJECT_ROOT" in sbm_test.group(1), "sbm-test doesn't use $PROJECT_ROOT variable"
    ...
    def test_sbm_test_runs_pytest(self):
        ...
        assert "pytest" in sbm_test.group(1), "sbm-test doesn't run pytest"
...
class TestSetupScriptWrapperValidation:
    ...
    def test_wrapper_validates_venv_exists(self):
        ...
        assert (
            '[ ! -f "$VENV_PYTHON" ]' in wrapper_text or '[ ! -f "\\$VENV_PYTHON" ]' in wrapper_text
    ...
    def test_wrapper_validates_project_root_exists(self):
        ...
        assert (
            ...
            or '[ ! -d "\\$PROJECT_ROOT" ]' in wrapper_text
    ...
    def test_wrapper_provides_recovery_instructions(self):
        ...
        assert "setup.sh" in wrapper_text, "Wrapper doesn't mention setup.sh in error recovery"
    ...
    def test_wrapper_changes_to_project_directory(self):
        ...
        assert 'cd "$PROJECT_ROOT"' in wrapper_text or 'cd "\\$PROJECT_ROOT"' in wrapper_text, (
...
class TestSetupScriptDocumentation:
    ...
    def test_setup_has_shebang(self):
        ...
        assert content.startswith("#!/bin/bash"), "setup.sh doesn't have proper shebang"
    ...
    def test_wrapper_generation_has_comments(self):
        ...
        wrapper_function = re.search(
            r"function create_global_wrapper.*?^}", content, re.DOTALL | re.MULTILINE
        )

        assert wrapper_function is not None, "create_global_wrapper function not found"
