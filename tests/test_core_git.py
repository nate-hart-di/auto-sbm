...
try:
    from sbm.core.git_operations import commit_changes, create_branch, push_branch
except ImportError:
...
class TestGitBasicOperations:
    ...
    def test_git_add_operations(self, tmp_path):
        ...
        # Add file
        result = subprocess.run(
            ["git", "add", "test.txt"], check=False, capture_output=True, text=True
        )
        assert result.returncode == 0
    ...
    def test_git_commit_operations(self, tmp_path):
        ...
        # Commit
        result = subprocess.run(
            ["git", "commit", "-m", "Test commit"], check=False, capture_output=True, text=True
        )
        assert result.returncode == 0

        # Check log
        result = subprocess.run(
            ["git", "log", "--oneline"], check=False, capture_output=True, text=True
        )
...
class TestGitBranchOperations:
    ...
    def test_branch_creation(self, tmp_path):
        ...
        result = subprocess.run(
            ["git", "checkout", "-b", branch_name], check=False, capture_output=True, text=True
        )
        assert result.returncode == 0
    ...
    def test_branch_switching(self, tmp_path):
        ...
        result = subprocess.run(
            ["git", "checkout", "master"], check=False, capture_output=True, text=True
        )
        if result.returncode != 0:
            # Try 'main' instead of 'master'
            result = subprocess.run(
                ["git", "checkout", "main"], check=False, capture_output=True, text=True
            )
...
class TestGitCommitWorkflow:
    ...
    def test_migration_commit_pattern(self, tmp_path):
        ...
        migration_files = ["sb-inside.scss", "sb-vdp.scss", "sb-vrp.scss"]
        ...
        result = subprocess.run(
            ["git", "commit", "-m", commit_msg], check=False, capture_output=True, text=True
        )
        assert result.returncode == 0
...
class TestGitRemoteOperations:
    ...
    def test_git_remote_check(self, mock_subprocess):
        ...
        result = subprocess.run(
            ["git", "remote", "-v"], check=False, capture_output=True, text=True
        )

        # Verify mock was called
        mock_subprocess.assert_called_with(["git", "remote", "-v"], capture_output=True, text=True)
...
class TestGitErrorHandling:
    ...
    def test_git_operations_with_no_commits(self, tmp_path):
        ...
        result = subprocess.run(["git", "log"], check=False, capture_output=True, text=True)
...
class TestGitIntegration:
    ...
    def test_complete_migration_git_workflow(self, tmp_path):
        ...
        # 3. Commit changes
        commit_msg = "Site Builder Migration: testtheme\n\nFiles generated:\n" + "\n".join(
            f"- {f}" for f in migration_files
        )
