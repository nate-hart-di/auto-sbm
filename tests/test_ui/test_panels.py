...
class TestStatusPanels:
    ...
    def test_migration_status_panel_basic(self):
        ...
        assert "test-theme" in str(panel)
        assert "Step 1" in str(panel)
        assert "✅" in str(panel)  # Success icon
    ...
    def test_migration_status_panel_with_additional_info(self):
        ...
        assert isinstance(panel, Panel)
        assert "⚠️" in str(panel)  # Warning icon
        assert "4/4" in str(panel)
        assert "2m 15s" in str(panel)
        assert "1" in str(panel)
    ...
    def test_docker_status_panel(self):
        ...
        assert isinstance(panel, Panel)
        assert "test-container" in str(panel)
        assert "Container started" in str(panel)
        assert "Compilation completed" in str(panel)
    ...
    def test_error_panel_basic(self):
        ...
        assert isinstance(panel, Panel)
        assert "Syntax Error" in str(panel)
        assert "test.scss" in str(panel)
        assert "42" in str(panel)
        assert "Undefined variable" in str(panel)
...
.header {
  ...
  margin: 10px;
        ...
        assert isinstance(panel, Panel)
        assert "$undefined" in str(panel)
        assert "Error here" in str(panel)
    ...
    def test_error_panel_with_suggested_fix(self):
        ...
        assert isinstance(panel, Panel)
        assert "Add semicolon" in str(panel)
        assert "Suggested Fix" in str(panel)
    ...
    def test_error_panel_syntax_highlighting_fallback(self):
        ...
        assert "INVALID SCSS" in str(panel)
    ...
    def test_git_status_panel_no_files(self):
        ...
        assert isinstance(panel, Panel)
        assert "test-theme" in str(panel)
        assert "test-branch" in str(panel)
