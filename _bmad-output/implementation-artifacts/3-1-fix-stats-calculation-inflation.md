...
- [x] Remove or disable estimation logic at `sbm/utils/tracker.py:378-383`
- [x] Update `get_migration_stats()` to return ONLY actual data from Firebase
- [x] Add flag/setting if estimation is needed for specific use cases (disabled by default)
- [x] Update tests to reflect new behavior (no estimation in stats display)
...
- [x] Run `python scripts/backfill_firebase_runs.py --dry-run` to preview changes
- [x] Verify it will populate the 40 MERGED PRs with `lines_migrated = 0`
- [x] Run `python scripts/backfill_firebase_runs.py --force` to apply changes
- [x] Verify all MERGED PRs now have accurate `lines_migrated` from GitHub
- [x] Document any PRs that still have 0 lines (invalid/closed PRs)
...
- [x] Run `python scripts/cleanup_firebase_garbage.py` to identify duplicates
- [x] Review duplicate runs (same slug, multiple success entries)
- [x] Determine if duplicates should be:
...
- [x] Document decision and implement cleanup if needed

### Task 5: Verify Accurate Stats Display (AC: CLI shows correct numbers)

- [x] Run `sbm stats` and verify numbers match actual Firebase data
- [x] Verify "Sites Migrated" = 164 (unique migrations)
- [x] Verify "Lines Migrated" = sum of actual `lines_migrated` from MERGED PRs
- [x] Verify "Time Saved" = Lines Migrated / 800 hours
- [x] Compare with `python -c "from sbm.utils.tracker import get_migration_stats; print(get_migration_stats())"`
- [x] Document baseline stats for future comparison
